"""GPT minimal (architecture GPT-2, style nanoGPT) — petit, entraînable sur CPU.

Embeddings token+position, blocs {LayerNorm pre-norm, CausalSelfAttention, MLP GELU}, poids de la tête
liés à l'embedding (weight tying). Config par défaut ~14M paramètres (n_layer=8, n_embd=384, n_head=8).
"""
...
class CausalSelfAttention(nn.Module):
    def __init__(self, cfg: GPTConfig):
        ...
        # masque causal (utilisé si scaled_dot_product_attention indispo)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            return logits, loss
        logits = self.lm_head(x[:, [-1], :])  # juste le dernier pas (génération)
        return logits, None
