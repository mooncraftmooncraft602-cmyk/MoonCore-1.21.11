...
public final class MechanicSubCommand implements SubCommand {
    ...
    private void create(CommandSender s, String[] a) {
        ...
        msg(s, "<green>Mécanique <white>" + id + "<green> créée. Définis son déclencheur : <white>/moon mechanic trigger "
                + id + " <USE_ITEM|INTERACT_BLOCK|BREAK_BLOCK|PLACE_BLOCK|KILL_ENTITY|DAMAGE_TAKEN|SNEAK|DEATH|RESPAWN|CONSUME_ITEM|FISH|PLAYER_JOIN|INTERVAL>");
    }
    ...
    private void help(CommandSender s) {
        ...
        msg(s, "<gray>  teleport, lightning, spawn_mob, title, clear_effects, feed, loot, launch, particle, broadcast, player_command, actionbar.");
        msg(s, "<gray>Placeholders (texte/title/command/broadcast/actionbar) : <white>%player% %world% %x% %y% %z% %online% %health% %food% %level%<gray>.");
    }
